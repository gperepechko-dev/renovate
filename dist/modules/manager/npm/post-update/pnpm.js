...
import { getPnpmWorkspaceMaturityExcludes, parsePnpmNoMatureMatchingVersions, shouldExcludeImmatureVersionForLockfileRetry, toMinimumReleaseAgeExcludeEntry, withPnpmMaturityExcludes } from "./pnpm-maturity.js";
...
async function generateLockFile(lockFileDir, env, config, upgrades = []) {
	...
	try {
		...
		if (await localPathExists(pnpmWorkspaceFilePath)) {
			...
			existingMaturityExcludes = getPnpmWorkspaceMaturityExcludes(pnpmWorkspace);
		...
		const maturityExcludes = [];
		let attemptCommands = commands;
		for (let attempt = 0; attempt <= 15; attempt++) try {
			...
			if (!immatureVersions.length) throw err;
			const newExcludeEntries = [];
			for (const immature of immatureVersions) {
				const excludeEntry = toMinimumReleaseAgeExcludeEntry(immature.packageName, immature.version);
				if (maturityExcludes.includes(excludeEntry) || existingMaturityExcludes.includes(excludeEntry)) continue;
				if (!shouldExcludeImmatureVersionForLockfileRetry({
			...
			attemptCommands = withPnpmMaturityExcludes(commands, maturityExcludes, existingMaturityExcludes);
			logger.debug({
				maturityExcludes,
				attempt: attempt + 1
