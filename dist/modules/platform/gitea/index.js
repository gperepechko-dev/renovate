...
import { setBaseUrl } from "../../../util/http/gitea.js";
...
function getLabelList() {
	if (config.labelList === null) {
		...
		const orgLabels = getOrgLabels(config.repository.split("/")[0], { memCache: false }).then((labels) => {
...
const platform = {
	async initPlatform({ endpoint, token }) {
		if (!token) throw new Error("Init: You must configure a Gitea personal access token");
		if (endpoint) {
			let baseEndpoint = trimTrailingApiPath(endpoint);
			baseEndpoint = ensureTrailingSlash(baseEndpoint);
			defaults.endpoint = baseEndpoint;
		} else logger.debug(`Using default Gitea endpoint: ${defaults.endpoint}`);
		setBaseUrl(defaults.endpoint);
		let gitAuthor;
		try {
			...
			if (defaults.version?.includes("gitea-")) {
				...
				logger.info(`Detected Forgejo instance, please use 'forgejo' platform instead`);
			}
			logger.debug(`${defaults.isForgejo ? "Forgejo" : "Gitea"} version: ${defaults.version}`);
		} catch (err) {
			logger.debug({ err }, "Error authenticating with Gitea. Check your token");
	...
	async setBranchStatus({ branchName, context, description, state, url: target_url }) {
		try {
			...
			await createCommitStatus(config.repository, branchCommit, {
				state: renovateToGiteaStatusMapping[state] || "pending",
				context,
				description,
	...
	async getBranchStatus(branchName, internalChecksAsSuccess) {
		...
		/* v8 ignore next */
		return giteaToRenovateStatusMapping[ccs.worstStatus] ?? "yellow";
	},
	async getBranchStatusCheck(branchName, context) {
		...
		if (!cs) return null;
		const status = giteaToRenovateStatusMapping[cs.status];
		if (status) return status;
		logger.warn({ check: cs }, "Could not map Gitea status value to Renovate status");
		return "yellow";
	},
	getPrList() {
		return GiteaPrCache.getPrs(giteaHttp, config.repository, config.ignorePrAuthor, botUserName);
	},
	async getPr(number) {
		...
		if (pr) logger.debug("Returning from cached PRs");
			...
			if (pr) await GiteaPrCache.setPr(giteaHttp, config.repository, config.ignorePrAuthor, botUserName, pr);
		}
		if (!pr) return null;
		return pr;
	},
	...
	async createPr({ sourceBranch, targetBranch, prTitle, prBody: rawBody, labels: labelNames, platformPrOptions, draftPR }) {
		...
		try {
			...
			if (platformPrOptions?.usePlatformAutomerge) if (semver.gte(defaults.version, defaults.isForgejo ? "10.0.0" : "1.24.0")) try {
				...
				logger.debug({ prNumber: gpr.number }, "Gitea-native automerge: success");
			} catch (err) {
				logger.warn({
					err,
					prNumber: gpr.number
			...
			else logger.debug({ prNumber: gpr.number }, `Gitea-native automerge: not supported on this version of ${defaults.isForgejo ? "Forgejo" : "Gitea"}. Use ${defaults.isForgejo ? "10.0.0" : "1.24.0"} or newer.`);
			const pr = toRenovatePR(gpr, botUserName);
			if (!pr) throw new Error("Can not parse newly created Pull Request");
			await GiteaPrCache.setPr(giteaHttp, config.repository, config.ignorePrAuthor, botUserName, pr);
			return pr;
		} catch (err) {
	...
	async addReviewers(number, reviewers) {
		...
		try {
			await requestPrReviewers(config.repository, number, { reviewers });
		} catch (err) {
			logger.warn({
				err,
				number,
