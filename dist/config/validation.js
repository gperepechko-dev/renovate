...
async function validateConfig(configType, config, isPreset, parentPath) {
	return instrument(`validateConfig(${configType}, ${JSON.stringify(parentPath)})`, async () => {
		initOptions();
		let errors = [];
		let warnings = [];
					...
					if (type === "boolean") {
						...
						if (key === "registryAliases") {
							...
							for (const [statusCheckKey, statusCheckValue] of Object.entries(val)) {
								...
								if (!isString(statusCheckValue) || !allowedWhenValues.includes(statusCheckValue)) {
									errors.push({
										topic: "Configuration Error",
