...
function handleGotError(err, url, opts) {
	...
	if (err.statusCode === 403 && message.includes("rate limit exceeded")) {
		...
		else if (parsed?.hostname === "api.github.com") logger.once.warn({ documentationUrl: `${GlobalConfig.get("productLinks").documentation}getting-started/running/#githubcom-token-for-changelogs-and-tools` }, `Rate limit exceeded for ${parsed.host}, as no hostRules set for this host. Please set a GITHUB_COM_TOKEN`);
